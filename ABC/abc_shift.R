...
library(here)
library(KScorrect)
source(paste(here(),'/ABC/abc_helper_shift.R',sep=""))
...
for(g in 1:G){  
  ...
  etol <- quantile(elist,0.4)
  print(paste0('New etol ', etol))
  write.csv(res.new, file = paste("log_adaptive_shift_",g,".csv",sep=""), row.names=FALSE)
}
